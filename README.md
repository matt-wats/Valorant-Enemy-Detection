...
After the key is pressed, it first centers your mouse (like how in Valorant your mouse is always in the center), then uses the trained model to detect enemies, then moves the mouse cursor onto the enemy (hopefully their head) with a 1/10th of a second delay, so that we can see it move.
